...
net.createServer((connection) =>{
	serverConnection(connection,9004)
}).listen(9004,console.log("Listening Scanner serial PCB"))

net.createServer((connection) =>{
	serverConnection(connection,9005)
}).listen(9005,console.log("Listening Scanner Serial Extrusion"))

function serverConnection(connection, port){
	console.log("Serial leido en el puerto: "+port)
	connection.on('data',function (data){
		console.log("soy data "+data.toString())
		console.log("soy el puerto.. ",port)
		if (data.toString().length > 10 && port == 9005){
			io.emit('Sequence_start',data.toString(),port-9004)
		}
		else if (port == 9005){
				console.log("Error al escanear extrusion")
			}
			else{
				data.toString().length > 10 && port == 9004 ? io.emit('Sequence_start',data.toString(),port-9004) : console.log("Error al escanear PCB")
			}
			
		// data.toString().length > 10 && port == 9005 || port == 9004 ? io.emit('Sequence_start',data.toString(),port-9004) : console.log("Error al escanear extrusion") 
		// data.toString().length > 10 && port == 9004 ? io.emit('Sequence_start',data.toString(),port-9004) : console.log("Error al escanear PCB")
		// data.toString() != "9004L: ERROR" ? io.emit('Sequence_start',data.toString(),port-9004) : "Error al escanear PCB"
	})
}
...
async function savingpicpruebas(datauri,point,sn,j){
	const ImageDataURI = require('image-data-uri');
	return new Promise(async (resolve, reject) => {
		try {
			let filePath = "samples-recortes/"+point+'-'+j+'';
			let filevalidation = fs.existsSync(filePath);
...
async function pass(sn, boxpoint) {
	return new Promise(async resolve => {
		...
		let client = net.connect(PORT, HOST, function () { //'connect' listener
			console.log(myJSON)
			client.write(myJSON)
...
async function validation(sn, csn) {
	return new Promise(async resolve => {
		...
		let PORT = 50000
		let HOST = '148.164.96.7'
		let myJSON = "F" + "&" + "Tokito Final Audit" + "&" + "" + sn + "" + "&" + "NA" + "&" + "Tesla-Tokito" + "&" + "tokito-V1" + "&" + "NA" + "&" + "NA" + "&" + "NA" + "&" + "NA" + "&" + "NA" + "&" + "0"
		console.log(myJSON)
		let client = net.connect(PORT, HOST, async function () { //'connect' listener
			console.log(myJSON)
			client.write(myJSON)
...
function workstation(serialnumber, pjct, unitnumber) {
	...
	let myJSON = JSON.stringify(post_data);
	...
	let post_req = http.request(post_options, function (chunk) {
